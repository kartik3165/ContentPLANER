...
async def get_top_competitor_posts(limit: int = 10) -> list[InstagramPost]:
    async with async_session_maker() as session:
        res = await session.execute(
            select(InstagramPost)
            .where(InstagramPost.post_owner_type == PostOwnerType.COMPETITION)
...
async def get_own_posts(limit: int = 10) -> list[InstagramPost]:
    async with async_session_maker() as session:
        res = await session.execute(
            select(InstagramPost)
...
async def update_engagement_scores():
    async with async_session_maker() as session:
        ...
        for post in res.scalars().all():
            post.engagement_score = calc_score(
